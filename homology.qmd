...
2022_2).](pics/trembl_stats.png "Number of entries in UniProt/TrEMBL")](https://www.ebi.ac.uk/uniprot/TrEMBLstats)
...
times less than the number of sequences (231,354,261 sequences for Uniprot and
192,489 structures in RCSB Protein Databank in 2022). This difference is called
...
2022).](pics/RCSB_stats.png "Number of macromolecular structures in RCSB PDB database.")](https://www.rcsb.org/stats/growth/growth-released-structures)
...
In 1968, Cyrus Levinthal (1922--1990) published the so-called *Levinthal's
paradox*, stating that proteins fold in nano/milliseconds, but even for small
peptides it will take a huge time to test the astronomical number of possible
conformations. Say a 100 aa small protein; it will have 99 peptidic bonds and
198 different phi and psi angles. Assuming only 3 alternative conformations for
each bond, it will yield 3^198^ (= 2.95 x 10^95^) possible conformations. If we
design a highly efficient algorithm that tests 1 conformation per nanosecond:
...
In this context, roughly 50 years ago, a very simple experiment led some light
on the protein folding mechanism. [Cristian
...
@anfinsen1973 .](pics/anfinsen.jpg "Anfinsen")
...
.](pics/funnel_1.jpeg "The funnel of protein folding")](https://www.sciencedirect.com/science/article/pii/S0968000400017072?casa_token=BR_maX_GNYYAAAAA:IZrQqI9jIbiv-ZeA6aOMHQxsVcp-wgy0XPFO3DRcFiAi7TSrX-3cc7Jb6dhTHdXSQseEhF3l#BIB49)
...
and computational resources.]{style="color:red"}

Homology modeling is one of the easiest tricks to bypass that limitation.
Basically, the strategy is adding all the possible extra information to the
amino acid properties, namely the evolutionary conservation of sequences and
structures.
...
prediction.](pics/predway.jpeg "Two different approaches for structure prediction.")](http://isw3.naist.jp/IS/Bio-Info-Unit/gogroup/study/study-en.html)
...
Bonvin
lab.](pics/rcsb-statistics.png "Protein structures and unique folds")](https://www.bonvinlab.org/education/molmod_online/modelling/)
...
[![CATH main categories exemplified the limited structures space, with 86k vs
500k domains in 2006 and 2022, respectively, but 40 and 41 architectures in the
same
years.](pics/cath.png "CATH")](https://www.cathdb.info/wiki/doku/?id=release_notes)
...
course.](pics/homology_simple_workflow.png "Homology Modeling workflow")](https://swissmodel.expasy.org/static/course/files/PartII_homology_modelling.pdf)
...
more detailed review on homology modeling, I suggest reading @haddad2020a.
...
requirement of a strong bioinformatics background or coding skills. [Rather than
provide just a recipe to make a model in two clics, the goal is to understand
the process behind, while doing exercises with selected
...
As an example, we are modeling a human DNA repair protein, **NEIL2,** and a
**viral DNA polymerase** (HAdV-2 pol).
...
biological units (protein structures classified by their quaternary structure).
...
[@kelley2009]](pics/template_query.png "Query-template alignment is the base for a new model")
...
[@kelley2009].](pics/profiles.jpg "From sequence vs. sequence search to profile-profile comparison")

Template searches in SWISS-MODEL are carried out with HHblits [@remmert2011], a
specific profile-profile method. We could search for templates also with Blast
...
are ranked by two different numeric parameters ranged between 0 and 1: **GMQE**
...
start](pics/seach.png "SWISS-MODEL modeling")](https://swissmodel.expasy.org/interactive)

[**Before building the models, could you foresee which of our queries will give
rise to a better model? Why?**]{style="color:green"}
...
alignment of the query and template sequence. SWISS-MODEL uses a fragment
assembly, as well as
[RosettaCM](https://new.rosettacommons.org/demos/latest/tutorials/rosetta_cm/rosetta_cm_tutorial),
another very well-known homology modeling algorithm by the [Baker
lab](https://www.bakerlab.org/), implemented in
[Rosetta](https://www.rosettacommons.org/software) software and the
[Robetta](https://robetta.bakerlab.org/) server. Other programs, like
[Modeller](https://salilab.org/modeller/), are based in the satisfaction of
spatial restraints [@janson2019].
...
Then, it will find compatible fragments in a dedicated loop database and also
use *ab initio* building or missing loops.

[![Backbone and loop modeling. From Expasy Protein Structure, Comparative
Protein Modelling, and
Visualisation.](pics/loops.png "Backbone and loop modeling")](https://swissmodel.expasy.org/static/course/files/PartII_homology_modelling.pdf)

Then, side chain of non conserved amino acids in undertook. The goal is findig
the most likely side chain conformation, using template structure information,
rotamer libraries (from known protein structures) and energetic and packaging
criteria. If many side chains have to be placed in the structure it will lead to
a "chicken and egg problem". Thus, the more residues correctly positioned, the
best model. That means that identification of hydrogen bonds between residues
side chains and between side chains and the backbone reduce the optimization
calculations.

[![Side chain modeling. From CMBI Seminars on
Bioinformatics](pics/Rotamers.gif "Backbone and loop modeling")](https://swift.cmbi.umcn.nl/teach/B1SEM/B1SEM_8.html)

Finally, a short energy minimization is carry out to reduce the unfavorable
...
Output models are colored in a temperature color scale, from navy blue (good
quality) to red (bad quality). That can help us to understand our model in a
first sight. Also, this is an interactive site and you can zoom-in, zoom-out the
model. Many other features are available to work on your model. For instance,
you can compare multiple models, you can change the display options. You can
also download all the files and reports on the "Project Data" button.

[![NEIL2 model created with SWISS-MODEL (July
2022)](pics/neil2.png "NEIL2 model")](https://swissmodel.expasy.org/interactive/TWq8LD/models/)

There is also a "Structure Assessment" option. This provide you a detailed
report of the structural problems of your model. You can see
[Ramachandran](https://en.wikipedia.org/wiki/Ramachandran_plot "Ramachandran plots in Wikipedia")
plots that highlight in red the amino acid residues with abnormal phi/psi angles
in the model and a detailed list of other problems.
...
The QMEANDisCO is a single parameter that combines statistical potentials and
agreement terms with a distance constraints (DisCo) to provide a consensus
score. DisCo evaluates consistencies of pairwise CA-CA distances from a model
with constraints extracted from homologous structures. All scores are combined
using a neural network trained to predict per-residue scores.

[![HAdV-2 DNA polymerase model obtained with
SWISS-MODEL.](pics/hadv-pol.png "HAdV-2 DNA polymerase")](https://swissmodel.expasy.org/interactive/GZ8GmU/models/)

QMEANDisCo can be used to analyze models obtained with other methods in order to
make them comparable. There are other model assessing tools commonly used to
assess protein models, like MoldFold [@mcguffin2021] or VoroMQA
[@olechnovi2017].

Another key parameter that you should know if you want to compare protein
strutures is the **alpha carbon RMSD** (root mean square deviation of the alpha
carbons), sometimes referred in the protein field only as as RMSD. Any protein
structural alignment will give you this parameter as an estimation of the
difference of the structures. You can align structure with many online servers,
like [FATCAT2](http://fatcat.godziklab.org/) or using molecular visualization
apps, like [ChimeraX](https://www.cgl.ucsf.edu/chimerax/) or
[Pymol](https://pymol.org/2/) (also available
[here](https://github.com/schrodinger/pymol-open-source)).

[**Which model is better? Which regions are more difficult to model?
...
.](pics/sali.jpeg "Accuracy and application of protein structure models.")](https://www.science.org/doi/10.1126/science.1065659)
